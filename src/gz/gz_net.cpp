...
int gzNet_c::connect(u32 ip, u16 port) {
    if (!g_gzInfo.mIsNintendont) {
        gzInfo_sendNotification("network requires nintendont");
        return -1;
    }

    // Layout: [4B cmd][4B ip][2B port][2B pad] = 12B, padded to 32
    u8 buf[32] ATTRIBUTE_ALIGN(32);
    memset(buf, 0, sizeof(buf));

    *(u32*)(buf + 0) = TPGZ_CMD_WORD(TPGZ_CMD_NET_CONNECT);
    buf[4] = (ip >> 24) & 0xFF;
    buf[5] = (ip >> 16) & 0xFF;
    buf[6] = (ip >> 8) & 0xFF;
    buf[7] = ip & 0xFF;
    ...
    if (!gzExiTransfer(buf, sizeof(buf), EXI_WRITE) ||
        !gzExiTransfer(statusBuf, sizeof(statusBuf), EXI_READ)) {
        gzInfo_sendNotification("connect: DMA failed");
        return -1;
    }
    ...
    if (status != 0) {
        char msg[48];
        snprintf(msg, sizeof(msg), "connect failed: status=%d", status);
        gzInfo_sendNotification(msg);
        return -1;
    }

    gzInfo_sendNotification("online: connected");
    return 0;
}

int gzNet_c::disconnect() {
    if (!g_gzInfo.mIsNintendont) {
        return -1;
    }
    ...
    u8 buf[32] ATTRIBUTE_ALIGN(32);
    memset(buf, 0, sizeof(buf));
    *(u32*)buf = TPGZ_CMD_WORD(TPGZ_CMD_NET_DISCONNECT);
    ...
    if (status != 0) {
        char msg[48];
        snprintf(msg, sizeof(msg), "disconnect failed: status=%d", status);
        gzInfo_sendNotification(msg);
        return -1;
    }

    gzInfo_sendNotification("online: disconnected");
    return 0;
}

int gzNet_c::stateWrite(const void* data, u32 len) {
    if (!g_gzInfo.mIsNintendont) {
        return -1;
    }
    ...
    // Layout: [4B cmd][payload...] padded to 32B
    u32 totalSize = DMA_ALIGN(4 + len);
    u8 buf[DMA_ALIGN(4 + 1400)] ATTRIBUTE_ALIGN(32);
    memset(buf, 0, totalSize);

    *(u32*)(buf + 0) = TPGZ_CMD_WORD(TPGZ_CMD_NET_STATE_WRITE);
...
int gzNet_c::stateRead(void* buf, u32 maxLen) {
    if (!g_gzInfo.mIsNintendont) {
        return -1;
    }
    ...
    *(u32*)cmdBuf = TPGZ_CMD_WORD(TPGZ_CMD_NET_STATE_READ);
    ...
    if (!gzExiTransfer(cmdBuf, sizeof(cmdBuf), EXI_WRITE) ||
        !gzExiTransfer(readBuf, readSize, EXI_READ)) {
        return -1;
    }
