...
# Import mod configuration (generated by setup_mod.py)
mod_config_path = Path(__file__).parent / "mod_config.py"
if mod_config_path.exists():
    spec = importlib.util.spec_from_file_location("mod_config", mod_config_path)
    mod_config = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod_config)
else:
    mod_config = None
...
config.libs = [
    ...
    # Extra dolphin SDK objects needed by the mod (not in the vanilla DOL).
    *(
        [
            {
                "lib": "mod_dolphin",
                ...
                "mw_version": "GC/1.2.5n",
                "cflags": cflags_dolphin,
                "objects": [Object(Custom, obj) for obj in
                    getattr(mod_config, "extra_objects", [])
                ],
            },
        ]
        ...
        [
            {
                ...
                "cflags": cflags_framework,
                "objects": [Object(Custom, obj) for obj in (
                    getattr(mod_config, "umbra_objects", [])
                    + getattr(mod_config, "mod_objects", [])
                )],
            },
        ]
...
def link_order_callback(module_id: int, objects: List[str]) -> List[str]:
    ...
    if not config.non_matching or mod_config is None:
        return objects
    if module_id == 0:  # DOL
        custom_objects = (
            getattr(mod_config, "extra_link_objects", [])
            + getattr(mod_config, "extra_objects", [])
            + getattr(mod_config, "umbra_objects", [])
            + getattr(mod_config, "mod_objects", [])
        )
...
if args.mode == "configure":
    ...
    # For non-matching builds with a mod config, add ISO rebuild step directly to build.ninja
    if config.non_matching and mod_config is not None:
        with open("build.ninja", "r") as f:
            content = f.read()

        iso_prefix = getattr(mod_config, "iso_prefix", "tpgz") if mod_config else "tpgz"
        output_iso = f"{iso_prefix}-{version}.iso"
